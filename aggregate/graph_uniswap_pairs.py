...
from ..utils.models.message_models import UniswapGraphPairSnapshot
from ..utils.models.message_models import UniswapGraphPairsSnapshot
...
class AggreagateGraphPairsProcessor(GenericProcessorAggregate):
    transformation_lambdas = None

    def __init__(self) -> None:
        self.transformation_lambdas = []
        self._logger = logger.bind(module='AggregateGraphPairsProcessor')
    ...
    async def compute(
        ...
        self._logger.info(f'Calculating graph pairs trade volume and reserves data for {msg_obj}')
        ...
        for pair in pair_data.values():
            graph_pairs.append(UniswapGraphPairSnapshot.parse_obj(pair))

        graph_pairs = sorted(graph_pairs, key=lambda x: x.liquidity, reverse=True)

        graph_pairs_snapshot = UniswapGraphPairsSnapshot(
            epochId=epoch_id,
            pairs=graph_pairs,
        )

        if not all(complete_flags):
            graph_pairs_snapshot.complete = False

        return graph_pairs_snapshot
